...
void integer_hashmap_memcheck_tester(){
    ...
    //map.print();
    //std::cout << map.curL << " " << map.size << std::endl;

//std::cout << "remove" <<std::endl; exit(0);
    //test: remove
    if(STATUS)std::cout<<c[4];
    for(int i=0;i<n;i+=3){
        map.remove(i);
        //std::cout << map.remove(i) << std::endl;
    }
    if(STATUS)std::cout<<c[0]<<std::endl;

    //map.print(); exit(0);

    //std::cout << map.curL << " " << map.size << std::endl; exit(0);
    ...
    for(int i=0;i<n;i++){
        ...
        if(STATUS == 0){
           // if (it == map.end()) std::cout << "end" << std::endl;
            if(it != map.end()) {
